...
#define WIND_WIDTH_SMS  1024
#define WIND_HEIGHT_SMS 672

#define WIND_WIDTH_GG 1280
#define WIND_HEIGHT_GG 860
#define OFFSET 3
...
static SDL_Renderer *theRenderer;
...
int main(int argc, char** argv)
{
    ...
    /* or using the default masks for the depth: */
        if (fsJoe == 1) {
            //Create The Surface in SW MEMORY OR CRASH
            if (MastEx&MX_GG) {
               thescreen=SDL_CreateRGBSurface(SDL_SWSURFACE ,width,height,8,0,0,0,0);
               //Create The Window as a fullscreen window with OPENGL
               theWindow=SDL_CreateWindow("DegaJoe with SDL 1.3",0,0,FULL_WIDTH,FULL_HEIGHT,SDL_WINDOW_OPENGL | SDL_WINDOW_FULLSCREEN);
            } else {
                 thescreen=SDL_CreateRGBSurface(SDL_SWSURFACE,width,height,8,0,0,0,0);
               //Create The Window as a fullscreen window with OPENGL
               theWindow=SDL_CreateWindow("DegaJoe with SDL 1.3",0,0,FULL_WIDTH,FULL_HEIGHT,SDL_WINDOW_OPENGL | SDL_WINDOW_FULLSCREEN);
            ...
            //Create the renderer accelerated
	          ...
	          thescreen=SDL_CreateRGBSurface(SDL_SWSURFACE ,GG_WIDTH,GG_HEIGHT,8,0,0,0,0);
               //Create The Window as a fullscreen window with OPENGL
               theWindow=SDL_CreateWindow("DegaJoe with SDL 2.0",0,0,width,height,SDL_WINDOW_OPENGL);
        
        }
        else if (fsJoe == 0) {
                thescreen=SDL_CreateRGBSurface(SDL_SWSURFACE ,SMS_WIDTH,SMS_HEIGHT,8,0,0,0,0);
               //Create The Window as a fullscreen window with OPENGL
               theWindow=SDL_CreateWindow("DegaJoe with SDL 2.0",0,0,width,height,SDL_WINDOW_OPENGL);
        }
        
        theRenderer=SDL_CreateRenderer(theWindow,-1,SDL_RENDERER_ACCELERATED | SDL_RENDERER_PRESENTVSYNC);
        SDL_GL_SetSwapInterval(1);//JoeFix 
	...
	{       
		...
		if(sound) while(audio_len>aspec.samples*aspec.channels*2*4) usleep(50);
                ...
                SDL_Texture *theTex=SDL_CreateTextureFromSurface(theRenderer,thescreen);
                //Clear the renderer
                ...
                if (MastEx&MX_GG) {
                     ...
                     SDL_RenderCopy(theRenderer,theTex,NULL,NULL);
                }
                else {   
                     SDL_RenderCopy(theRenderer,theTex,NULL,NULL);
                }
                ...
                if (getenv("JOEFORCE3D") != NULL) {
                   //FORCE 3D RENDERING
                   SDL_RenderClear(theRenderer);
                   //offset the right image to OFFSET
                   SDL_Rect offRect;
                   offRect.x=OFFSET;
                   offRect.y=0;
                   
                   offRect.w=FULL_WIDTH-OFFSET;
                   offRect.h=FULL_HEIGHT;  
                    //render right image
                   SDL_RenderCopy(theRenderer,theTex,NULL,&offRect);
                   SDL_RenderPresent(theRenderer);
                   //Destroy texture
                }
                SDL_DestroyTexture(theTex);
                //Present on scrreen
...
#define p(x) Mdraw.Pal[x]
		...
		SDL_SetPaletteColors(pal, themap, 0, 256);
                SDL_SetSurfacePalette(thescreen,pal);
	}
